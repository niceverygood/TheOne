...
슬롯마다 한 장씩 열리던 것을 **두 장씩**으로 늘렸다(12·15·20시 × 2 = 하루 6장).

- **`releasedCardCount(now, hours, perSlot)` + `DEFAULT_CARDS_PER_SLOT = 2`** — 열린 슬롯 수 × 슬롯당 장수. 슬롯 시각 계산은 그대로 재사용한다.
- **`curationCardsPerSlot()`** — env `CURATION_CARDS_PER_SLOT`(1~10, 벗어나면 기본값 2). `curationQuotaNow()` 가 이 값을 곱해 쿼터를 낸다.
- **`/api/curation/today`** 의 `slots` 에 `perSlot` 추가 → 모바일 안내 문구가 "12:00 · 15:00 · 20:00에 2장씩 도착합니다"로 실제 설정을 말한다(구버전 응답이면 2로 폴백).
- **크론 푸시 문구**가 실제 배정 장수를 말한다 — "새로운 카드 2장이 도착했습니다". `CurationSlotDelivery.added` 추가.

**작업 중 잡은 결함**: `getTodayCurations(userId, now)` 가 `now` 를 받아 하루 경계·쿼터를 계산하면서도 `CurationLog.sentAt` 은 DB 기본값 `now()` 로 남겨, 호출자가 시각을 주입하면 "오늘 발송분" 복원 쿼리가 방금 만든 카드를 못 찾아 **매 호출마다 쿼터만큼 새로 만들었다**(시각 주입 시나리오에서 하루 로그 23건). `sentAt: now` 로 못박아 해결 — 같은 시나리오에서 6건으로 정상화.

**검증**: 실 DB — 11:30 KST 0장 → 12:30 2장 → 14:30 2장 → 15:30 4장 → 19:30 4장 → 20:30 6장 → 23:30 6장, 하루 총 로그 6건. shared 57 tests(슬롯당 장수·범위 밖 값 폴백 포함), `turbo typecheck lint test` 11/11.


## 탐색 구조 개편 — 하단 탭 5개 + 회원 둘러보기

큐레이션 한 화면에서만 밀고 들어가던 구조를 상용 매칭앱처럼 탭으로 바꿨다. **외관은 그대로다** — 아이보리 바탕·상단 hairline·그림자 없음·활성 표시는 잉크블랙 텍스트와 샴페인 점 하나. §3 금기(골드 그라데이션·하트·라운드 카드·그림자)는 하나도 건드리지 않았다.

- **`app/(tabs)/_layout.tsx`(신규)** — 오늘·둘러보기·매칭함·지난 카드·마이. `curation`·`inbox`·`history`·`my` 를 그룹으로 이동(그룹은 URL 세그먼트가 아니라 기존 링크·딥링크 경로는 그대로).
- **`app/(tabs)/browse.tsx`(신규) — 둘러보기**: 2열 그리드, 지역·나이대·인증 필터 칩, 무한 스크롤(커서). **카드 덱·스와이프·좋아요 카운터 없음** — 카드에서 가는 행동은 '만남 신청' 하나뿐이고, 이미 보낸 상대는 '신청함'으로 잠근다. 인증 뱃지는 목록에서도 노출(더원의 본질).
- **`browseMembers()` + `GET /api/members/browse`** — 이성·활동 회원만, 본인·차단(양방향)·QA 계정 제외. 정렬은 **최근 가입순**(점수순으로 줄 세우면 상위 몇 명에게 신청이 쏠린다). 목록에는 대표 사진 1장만, 단계별 공개는 프로필 상세가 담당.
- **탭 루트 정리** — 마이·지난 카드·매칭함의 뒤로가기 화살표와 큐레이션·매칭함의 `☰ MENU` 버튼 제거(하단 탭이 대신한다).

**작업 중 잡은 결함**: `verifiedOnly` 를 DB 20건 조회 후 앱단에서 걸러, 인증 회원이 더 있어도 목록이 1건에서 끊기고 `nextCursor` 도 null 이 됐다. Prisma 관계 조건(`badges: { some: { expiresAt: { gt: now } } }`)으로 옮겨 해결 — 같은 조건에서 1건 → 6건으로 정상화.

**검증**: 실 DB — 전체 1페이지 20건 + 2페이지 6건(커서 전진), 지역=서울&30대 → 6명(나이 31~37, 지역 전부 서울), 인증 회원만 → 6명(뱃지 0인원 0), 나이 99+ → 0건, 미인증 401. `turbo typecheck lint test` 11/11, web `next build`·모바일 `expo export` OK.


## 지난 카드 — 지나간 인연 다시 보기

큐레이션은 매일 사라지지만, 소개받았던 분을 다시 찾아볼 수 없는 건 손실이다. 이력을 되짚는 화면을 붙였다. **새 후보를 만들지 않고 이미 발송된 `CurationLog` 만 되짚는다** — 무한 스와이프가 되지 않는다.

- **`getCurationHistory(userId, { cursor, limit, includeToday })`** — 최신순 커서 페이지네이션. 사진은 지금 시점의 공개 단계(별점·상호호감·매칭)만큼만, 탈퇴한 후보는 이력에서도 감춘다. 내가 그분께 보낸 신청서 상태(pending/accepted/declined)를 함께 반환.
- **`GET /api/curation/history`** — 세션 인증. 기본은 오늘 카드 제외(오늘 것은 큐레이션 화면이 이미 보여준다), `includeToday=1` 로 포함 가능.
- **`app/history.tsx`(신규)** — 날짜(오늘/어제/8월 12일 (수))·직무·나이·지역·대표사진 목록, 무한 스크롤. 카드를 누르면 기존 프로필 상세로 이동해 신청서까지 이어진다. 차단한 회원은 이력에서도 숨긴다.
- **진입점** — 마이페이지 '매칭 > 지난 카드', 오늘 카드를 다 본 화면의 '지난 카드 보기'.
- **오늘 카드도 되돌아갈 수 있다** — 기존에는 다음 후보로만 넘어가 앞서 본 분으로 돌아올 수 없었다. `← 앞서 본 분` 이동 추가, Pass 안내 문구도 "다시 표시되지 않습니다" → "지난 카드에서 다시 보실 수 있습니다"로 정정(실제 동작과 일치).

**검증**: 실 DB — 오늘 카드는 이력에서 제외(0건), 과거로 옮기면 2건 노출, 신청서 상태(accepted) 동반. 과거 로그 27건으로 페이지네이션 20+7·커서 전진 확인. 미인증 401·잘못된 커서 400. 커서와 '오늘 제외'가 둘 다 `sentAt` 상한이라 서로 덮어쓰던 문제는 하나로 합쳐 해결.

## 카드 추천 하루 3회 슬롯 (12·15·20시 KST)

무한 스와이프를 하지 않는다는 원칙을 스케줄로 못박았다. 카드는 "오늘 N장"이 아니라 **정해진 시각에 한 장씩** 열린다.

- **`packages/shared/curation-slots.ts`(신규)** — 슬롯 시각·KST 하루 경계 계산의 단일 출처. `kstDayStart`·`releasedSlotCount`·`nextSlotAt`·`countdownTo`·`parseSlotHours`. 서버가 UTC 로 돌아도 하루가 KST 자정에 끊긴다(기존엔 서버 로컬시간이라 한국 사용자의 하루가 09:00 에 바뀌었다). 테스트 8건.
- **`getTodayCurations(userId, now)`** — 하루 총량이 `CURATION_PER_DAY`(env) 가 아니라 **지금까지 열린 슬롯 수**. 12시 전 0장 · 12시 1장 · 15시 2장 · 20시 3장. 이미 만든 카드는 그대로 두고 부족분만 채운다.
- **`runCurationSlot()`** — 하루 3회 호출되는 CRON 러너. 같은 슬롯에 두 번 돌아도 카드가 늘지 않고(멱등), **이번 호출에서 새로 카드를 받은 회원만** 반환해 푸시 중복을 막는다. `runDailyCuration` 은 별칭으로 유지.
- **`/api/cron/curation`** — 슬롯 러너 호출 + 새 카드 수령자에게 Expo 푸시. `vercel.json` 크론 3개(03:00·06:00·11:00 UTC = 12·15·20 KST).
- **`/api/push/register`(신규)** + `User.pushToken` 컬럼(마이그레이션 `20260811000000_user_push_token`) — 세션 인증으로 본인 토큰만 저장, 형식 검증(`ExponentPushToken[...]`).
- **`/api/curation/today`** 가 `slots { hours, released, nextAt }` 동반 반환.
- **모바일** — 헤더 카운트다운이 자정이 아니라 **다음 카드 시각**(`NEXT CARD`, 1분 갱신). 첫 슬롯 전에는 "오늘의 첫 카드는 12:00에 도착합니다" + 슬롯 안내. 로컬 알림도 19시 1회에서 **12·15·20시 3회**로 교체(서버 푸시가 닿지 않는 환경 보완). 로그인 시 Expo 토큰 자동 등록.

**검증**: 실 DB 로 시각을 11:30~23:30 KST 로 바꿔 가며 quota 0→1→1→2→2→3→3, 하루 총 로그 3건 확인. CRON 2회 연속 실행 시 2번째 `sent: 0`(멱등). `/api/push/register` 401·400·200 확인. `turbo typecheck lint test` 11/11(shared 55 tests), web `next build`·모바일 `expo export` OK.

**운영 필요**: ① `push_token` 마이그레이션 실행 ② Vercel 환경변수 `CURATION_SLOT_HOURS="12,15,20"`(기본값과 같아 생략 가능), 더 이상 쓰지 않는 `CURATION_PER_DAY` 는 삭제 권장 ③ Android 원격 푸시는 EAS 에 FCM 자격증명 필요(미설정 시 로컬 알림만 동작).
